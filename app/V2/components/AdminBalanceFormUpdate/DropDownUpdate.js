...
const DropDown = ({ json, name, register, errors, clickDropDown, open }) => {
  const handleClick = () => {
    if (json != undefined) clickDropDown(json.idTipoBalanceMasa);
    // console.log(json.idTipoBalanceMasa);
  };
          ...
          <div key={item.idBalanceMasa}>
            <div className="form-group-dropdown" key={item.idBalanceMasa}>
              ...
              <input
                ...
                placeholder="Value"
                defaultValue={item.valor ? item.valor.replace(',', '.') : ""}
              ...
              <input
                ref={register()}
                type="hidden"
                value={item.idFlujoBalanceMasa}
                key={item.idFlujoBalanceMasa}
                name={'idFlujoBalanceMasa' + item.idFlujoBalanceMasa}
              />
            </div>
