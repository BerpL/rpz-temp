...
function ContainerAdminFolderAccess() {
  ...
  const [tempGrupos, setTempGrupos] = useState([]);
  ...
  const getSelectGrupos = async (idGrupo, idArbol) => {
    setIsLoading(true);
    try {
      const response = await groupService.getGrupos(idGrupo, idArbol);
      const { data } = response;
  ...
  const handleClickSave = async () => {
    ...
    grupos.forEach(grupo => {
      jsonGrupo.push({ idArbol: grupo.id, acceso1: grupo.habilitado });
    });

    // console.log(jsonGrupo);

    try {
      const response = await groupService.updateGroup(value, jsonGrupo);
      setIsSaved(true);
...
ContainerAdminFolderAccess.propTypes = {};

export default ContainerAdminFolderAccess;
