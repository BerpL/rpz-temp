...
function ListAdminReports({ users, identifierContextMenu, onOpen }) {
  return (
    <C1>
      <C2>
        <S1 direction="column" noPadding>
          <Header style={{ padding: '0px 16px' }}>
            <Column flex={6}>First Name</Column>
            <Column flex={6}>Last Name</Column>
            <Column flex={4}>User</Column>
            <Column flex={4}>Rating (%)</Column>
            <Column flex={2} />
          </Header>
          <Co02>
            {users.map(item => (
              ...
              >
                <Row onDoubleClick={() => onOpen(item.type, item.id)}>
                  ...
                  <Column flex={2}>
                    <ButtonAdminAction>Details</ButtonAdminAction>
                  </Column>
                </Row>
              </TreeAdminContextMenu>
            ))}
