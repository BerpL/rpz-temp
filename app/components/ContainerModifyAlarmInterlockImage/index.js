...
import { getInterfaceTags } from 'services/InterfacesTagsService';
...
function ContainerModifyFolder({ id, onClose, onModify, uploadProgress }) {
  const {
    values,
    handleChange,
    handleSubmit,
  ...
  async function createOrUpdate() {
    ...
    const close = await onModify(
      values,
      values.imagen ? values.imagen[0] : null,
    );
    if (close) {
      onClose();
  ...
  const imagePath = values.urlImagen
    ? values.urlImagen.replace(/\\/g, '/').trim()
    : '';
  ...
  return (
    <Container>
      <InnerContainer>
        <Title>{id ? 'Edit Image' : 'New Image'} </Title>
...
export default memo(ContainerModifyFolder);
