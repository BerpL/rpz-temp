...
const Tag = ({ tag, enableRemove, onRemove, selected }) => {
  // console.log(tag);
  const enclavamiento = tag.idEnclavamientoAlarmaNavigation;
  return (
    <Container>
      <Name>{`${enclavamiento.idItem} ${enclavamiento.instrumento}`}</Name>

      <CloseIcon onClick={() => onRemove(tag.idDetalleEtiquetaModulo)}>
        <MdClose />
      </CloseIcon>
    </Container>
  );
